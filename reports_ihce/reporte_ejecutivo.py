...
class reporte_ejecutivo(osv.osv_memory):
    ...
    def create_principal_sheet(self, cr, uid, ids, sheet, context={}):
        ...
        register_idss = self.pool.get('register.trademark').search(cr, uid, [('servicio','=','True'),('date','>=',data.date_ini),('date','<=',data.date_fin),('option','=','emprered')], order='date ASC')
        ...
        for row in self.pool.get('date.courses').browse(cr, uid, courses_ids, context=None):
            ...
            for line in self.pool.get('company.line').search(cr, uid, [('course_id','=',row.id)], context=None):
                ...
                if li.company_id.id != False:
                    #pdb.set_trace()
                    for ro in self.pool.get('companies.ihce').search(cr, uid, [('id','=',li.company_id.id)], context=None):
                        #hombre = hombre + 1
                        row = self.pool.get('companies.ihce').browse(cr, uid, ro)
                        if row.id != False:
                            if str(row.sexo) == 'M':
                                hombre = hombre + 1
                            if str(row.sexo) == 'F':
            ...
            for line2 in self.pool.get('list.new.persons').search(cr, uid, [('course_id','=',row.id)]):
                li = self.pool.get('list.new.persons').browse(cr, uid, line2)
                mujer = mujer + 1

        inicio = mujer + hombre
            
        if asistentes != inicio:

            faltante = asistentes - inicio

            hombre = hombre + faltante


        
        #asistentes = hombre + hombre
        sheet.write_merge(64, 64, 6, 9, horas, style_n)
        sheet.write_merge(65, 65, 6, 9, asistentes, style_n)
        sheet.write_merge(66, 66, 6, 9, "Hombres: "+str(hombre)+"               Mujeres: " + str(mujer) , style_B)
