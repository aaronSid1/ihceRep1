...
class report_emprendimiento_emprered(osv.osv_memory):
    _name = "report.emprendimiento.emprered"
    ...
    def action_create_report(self, cr, uid, ids, context=None):
        ...
        sheet_principal = workbook.add_sheet('Emprendimiento Emprered', cell_overwrite_ok=True)

        # Creamos la Hoja principal
        self.create_principal_sheet(cr, uid, ids, sheet_principal, context)
        # Creamos el nombre del archivo
        name = "EmprendimientoEmprered.xls"
        ...
        data_attach = {
            ...
            'datas': sprint_file,
            'datas_fname': name,
            'description': 'Reporte Emprendimiento Emprered',
            'res_model': 'report.emprendimiento.emprered',
            'res_id': ids[0],
        }
    ...
    def create_principal_sheet(self, cr, uid, ids, sheet, context={}):
        ...
        sheet.write(5, 8, 'Registro en la SHCP', style)
        sheet.write(5, 9, 'MES DE REGISTRO', style)
        ...
        if data.type == 'completo':
            em_ids = self.pool.get('asesorias.ihce').search(cr, uid, [('option','=','emprered'),('name','=','emprendimiento')], order='date ASC')
        else:
            em_ids = self.pool.get('asesorias.ihce').search(cr, uid, [('option','=','emprered'),('name','=','emprendimiento'),('date','>=',data.date_ini),('date','<=',data.date_fin)], order='date ASC')
        ...
        for row in self.pool.get('asesorias.ihce').browse(cr, uid, em_ids, context):
            ...
            sheet.write(i, 2, company.name.encode('utf-8')  or '', style_n)
