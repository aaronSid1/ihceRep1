...
class reporte_recepcion(osv.osv_memory):
    _name = "reporte.recepcion"
    ...
    def create_principal_sheet(self, cr, uid, ids, sheet, context={}):
        ...
        #CABECERA
        sheet.write_merge(0, 0, 0, 14,("Recepción), styleT)
        sheet.write_merge(1, 1, 0, 14,("Reporte de ingreso al IHCE"), styleT)
        #TITULOS
        sheet.write(3, 0, 'ID', style)
        sheet.write(3, 1, 'NOMBRE COMPLETO', style)
        sheet.write(3, 2, 'EDAD', style)
        sheet.write(3, 3, 'SEXO', style)
        sheet.write(3, 4, 'TELEFONO CEALULAR', style)
        sheet.write(3, 5, 'CORREO', style)
        ...
        for row in self.pool.get('date.courses').browse(cr, uid, courses_ids, context):
            ...
            for line in self.pool.get('list.new.persons').search(cr, uid, [('course_id','=',row.id)]):
                ...
                sheet.write(m, 5, li.name.encode('utf-8') or '', style_n)
