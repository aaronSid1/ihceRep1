...
class reporte_servicios_emprered(osv.osv_memory):
    ...
    def create_principal_sheet(self, cr, uid, ids, sheet, context={}):
        ...
        if(data.emprered.id != False):
            ...
            emprered = self.pool.get('emprereds').search(cr, uid, [('id','=',data.emprered.id)])

            for row in self.pool.get('emprereds').browse(cr, uid, emprered, context=None):
                emprered = row.name 
                pdb.set_trace()

            
            sheet.write_merge(3, 3, 0, 13,(emprered), styleT)
            ...
            if data.type == 'completo':
                register_ids = self.pool.get('register.trademark').search(cr, uid, [('servicio','=','True'),('option','=','emprered'),('emprered','=',data.emprered.id)], order='date ASC')
                bar_ids = self.pool.get('bar.code').search(cr, uid, [('servicio','=','True'),('option','=','emprered'),('emprered','=',data.emprered.id)], order='date ASC')
                fin_ids = self.pool.get('servicios.ihce').search(cr, uid, [('option','=','emprered'),('name','=','financiamiento'),('emprered','=',data.emprered.id)], order='date ASC')
                manos_ids = self.pool.get('servicios.ihce').search(cr, uid, [('option','=','emprered'),('name','=','manos'),('emprered','=',data.emprered.id)], order='date ASC')
                ade_ids = self.pool.get('servicios.ihce').search(cr, uid, [('option','=','emprered'),('name','=','adecuacion'),('emprered','=',data.emprered.id)], order='date ASC')
            else:
                register_ids = self.pool.get('register.trademark').search(cr, uid, [('servicio','=','True'),('date','>=',data.date_ini),('date','<=',data.date_fin),('option','=','emprered'),('emprered','=',data.emprered.id)], order='date ASC')
                bar_ids = self.pool.get('bar.code').search(cr, uid, [('servicio','=','True'),('date','>=',data.date_ini),('date','<=',data.date_fin),('option','=','emprered'),('emprered','=',data.emprered.id)], order='date ASC')
                fin_ids = self.pool.get('servicios.ihce').search(cr, uid, [('name','=','financiamiento'),('date','>=',data.date_ini),('date','<=',data.date_fin),('option','=','emprered'),('emprered','=',data.emprered.id)], order='date ASC')
                manos_ids = self.pool.get('servicios.ihce').search(cr, uid, [('name','=','manos'),('date','>=',data.date_ini),('date','<=',data.date_fin),('option','=','emprered'),('emprered','=',data.emprered.id)], order='date ASC')
                ade_ids = self.pool.get('servicios.ihce').search(cr, uid, [('name','=','adecuacion'),('date','>=',data.date_ini),('date','<=',data.date_fin),('option','=','emprered'),('emprered','=',data.emprered.id)], order='date ASC')
